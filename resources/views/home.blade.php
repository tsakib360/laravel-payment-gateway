                    ...
                    <form action="{{ route('pay') }}" method="POST" id="paymentForm">
                        ...
                        <div class="row">
                            <div class="col-auto">
                                ...
                                <small class="form-text text-muted">Use values with up to two decimals positions, using a "."</small>
                            </div>

                            <div class="col-auto">
                                <label>Currency</label>
                                <select name="currency" class="custom-select" id="currencySelect" required>
                                ...
                                <div class="form-group" id="toggler">
                                    <div class="btn-group btn-group-toggle" data-toggle="buttons">
                                        @foreach ($paymentPlatforms as $paymentPlatform)
                                            <label data-currency="{{ $paymentPlatform->name }}" class="btn btn-outline-secondary rounded m-2 p-1" data-target="#{{ $paymentPlatform->name }}Collapse" data-toggle="collapse">
                                                <input type="radio" name="payment_platform" value="{{ $paymentPlatform->id }}" required>
                                                <img src="{{ $paymentPlatform->image }}" class="img-thumbnail" alt="{{ $paymentPlatform->name }}">
                                            </label>
                                        @endforeach
                                    </div>
                                    @foreach ($paymentPlatforms as $paymentPlatform)
...
<script>
    $('#currencySelect').on('change', function () {
       var currencyValue = $('#currencySelect').val();
